...
def hb(f, tag, content1, content2=None, content3=None):
  ...
  if content2 is None:
    # Use str.replace (literal) instead of re.sub, whose replacement string
    # would otherwise interpret backslashes (e.g. '\h' from external links).
    r = tag.replace('|', content1)
    r = r.replace('|3', content3)
    r = mathjaxeqresub(r)
    out(f, r)
  else:
    r = tag.replace('|1', content1)
    r = r.replace('|3', content3)
    r = r.replace('|2', content2)
    r = mathjaxeqresub(r)
    out(f, r)
...
def replacelinks(b):
  ...
  while m:
    ...
    if m1[0] == '/':
        ...
        # Escape the quotes so the later "smart-quote" pass (which turns
        # unescaped " into curly quotes) leaves them alone; the trailing
        # backslash-strip restores them to plain ".
        option = ' target=\\"blank\\"'
